 ...
 */
  ...
  for (q = 0; q < NR_SCHED_QUEUES; q++) {	
	/**
	 * To modify the code so that the login runs slower, I first
	 * used the rand function to get a random number from 0-9, then
	 * make the condition greater than 8. This will give a one in
	 * 10 chance for the q modification to happen. After that, I make
	 * q > 3 so that it does not modify the ones with highest priority.
	 * Then if these condiitions are met, I use the rand function and
	 * mod NR_SCHED_QUEUES so that it modulos the difference up to
	 * whatever NR_SCHED_QUEUES is set to.	
	 */
 	if((rand() % 10 > 8) && (q > 3))
	{
		q = rand() % (NR_SCHED_QUEUES - q);
	}
...
>>>>>>> d9a142390054a3dad83fa2e228055af63f6152f9
